...
$data = array();
if (!empty($id)) {
	//$sql = "SELECT * FROM " . __TABLE_DATA__ . " WHERE id=$id";
	$sql = "SELECT a.*,b.iso FROM " . __TABLE_DATA__
...
if ($act == 'save') {
	...
	if (!empty($id)) {
		if ($db->where("id=" . $id)
			->update(__TABLE_DATA__ , $data)){
			$_succeed = TRUE;	
		}
	}
	else {
		if ($db->insert(__TABLE_DATA__ , $data)) {
			$_succeed = TRUE;
		}
	}
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="list_row" style="background:#e8e8e8">
    ...
    <tr bgcolor="#F0FFF0">
        ...
        <button id="forward" type="button" onclick="window.location.href='<?php echo $_COOKIE['referer']?>'" class="button btn-cart">
