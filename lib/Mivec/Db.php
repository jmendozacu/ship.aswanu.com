...
/*        if (is_file("../config/config.inc.php")){   //检查数据库参数文件是否存在，配置文件
            include("../config/config.inc.php");
            $this->_host = $conf['dbhost'];
            $this->_port = $conf['dbport'];
            $this->_user = $conf['dbuser'];
            $this->_pass = $conf['dbpass'];
    ...
    public function select($tbName='') {
        $this->_sql = "select ".trim($this->_field)." from ".$tbName." ".trim($this->_where)." ".trim($this->_order)." ".trim($this->_limit);
        $this->_clear = 1;
        $this->_clear();
        return $this->_doQuery(trim($this->_sql));
    }
